...
def active_train(model_type,
    ...
    for round in trange(math.ceil(num_acquisitions / acquisition_batch_size)):
        ...
        optimizer = optimizer_type(model.parameters(),
                                   lr=0.001,
                                   weight_decay=l2_penalty / len(train_pool))
...
def active_iteration_dkl(model,
                         likelihood,
                         epochs,
                         optimizer,
                         train_loader,
...
def eval_dkl(model, likelihood, test_loader, mc_dropout_iterations):
    ...
    with torch.no_grad(), gpytorch.settings.num_likelihood_samples(mc_dropout_iterations):
        for data, target in test_loader:
            preds = likelihood(
                model(data))    # This gives us 16 samples from the predictive distribution
    print(preds)
    print(dir(preds))
...
def plot(name: str,
         metrics: List[float],
         save_dir: str,
         acquisition_fn_type: str,
         **kwargs) -> None:
    df = pd.DataFrame({
        name: [item for item in metrics],
    ...
    fig = px.line(df,
                  x='iteration',
                  y=name,
                  title=f'{acquisition_fn_type}: {name} as a Function of Iteration')
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Create and run active learning experiments on 5 prime splicing data')
    ...
    parser.add_argument('--save_dir',
                        default='./morio-model-runs/',
                        help='path to saved model files')
    parser.add_argument('--checkpoint_path',
                        default='./morio-model-runs/foo.pt',
    ...
    for iter in range(configs['num_repeats']):
        ...
        with open(
                Path(
                    Path.home(),
                    configs['save_dir'],
                    f'{configs["acquisition_fn_type"]}_iteration_{iter}-batch_size-{configs["acquisition_batch_size"]}.json'
                ),
                'w') as f:
            json.dump(mse, f)
